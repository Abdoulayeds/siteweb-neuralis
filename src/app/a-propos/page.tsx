...
export default function AProposPage() {
  return <>
    ...
    <section className="studio-section"><div className="studio-container grid items-center gap-10 lg:grid-cols-2">
      ...
      <figure className="studio-panel themed-card overflow-hidden" data-tone="cyan"><Image src="/images/services/consulting.webp" alt="Photographie de professionnels échangeant autour d’un projet" width={1200} height={800} sizes="(min-width: 1024px) 50vw, 100vw" className="aspect-video w-full object-cover" /><figcaption className="border-t border-slate-200 p-5 text-sm leading-7 text-slate-600">Comprendre votre réalité avant de choisir la technologie. Photographie d’illustration.</figcaption></figure>
    </div></section>
    <section id="methode" className="studio-section scroll-mt-28 border-y border-white/10"><div className="studio-container">
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{steps.map((step,index) => <article key={step.title} data-tone={["cyan", "indigo", "mint", "orchid"][index]} className="studio-panel themed-card p-6"><span className="font-mono text-sm text-cyan-800">0{index + 1}</span><h3 className="mt-5 text-lg font-semibold text-slate-900">{step.title}</h3><p className="studio-copy mt-3 text-sm">{step.summary}</p></article>)}</div>
    </div></section>
    <section className="studio-section"><div className="studio-container"><div className="max-w-2xl"><p className="studio-kicker">Nos principes</p><h2 className="studio-heading mt-3">Des engagements dans la façon de travailler.</h2></div><div className="mt-10 grid gap-5 md:grid-cols-2">{principles.map((principle,index) => <article key={principle.title} data-tone={["mint", "cyan", "amber", "indigo"][index]} className="studio-panel themed-card flex gap-5 p-6 md:p-8"><span className="themed-card-icon shrink-0"><principle.icon className="h-7 w-7" strokeWidth={1.5} aria-hidden /></span><div><h3 className="text-xl font-semibold text-slate-900">{principle.title}</h3><p className="studio-copy mt-3 text-sm">{principle.text}</p></div></article>)}</div></div></section>
    <section className="studio-section border-t border-white/10"><div className="studio-container"><div className="studio-panel themed-card flex flex-col gap-7 p-8 md:flex-row md:items-center md:justify-between md:p-12" data-tone="indigo"><div className="max-w-2xl"><p className="studio-kicker">Construisons la suite</p><h2 className="mt-3 text-3xl font-semibold text-slate-900">Parlons de ce que vous voulez rendre possible.</h2><p className="studio-copy mt-4">Une idée précise ou un problème encore flou : les deux sont de bons points de départ.</p></div><ButtonLink href="/contact">Rencontrer NEURALIS</ButtonLink></div></div></section>
  </>;
}
