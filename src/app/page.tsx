  ...
  return <>
      ...
      <div className="studio-container">
        <div className="grid items-center gap-10 lg:grid-cols-[1.05fr_1fr] lg:gap-12">
          <div>
            ...
            <h1 className="hero-title">Une présence qui marque.<br /><span>Des outils qui font avancer.</span></h1>
            <p className="studio-copy mt-6 max-w-xl">Sites web, applications, plateformes et solutions data ou IA : nous construisons le digital autour de vos besoins réels, avec vous et pour vos utilisateurs.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><ButtonLink href="/contact">Parlons de votre projet</ButtonLink><ButtonLink href="#expertises" variant="ghost">Voir nos expertises</ButtonLink></div>
            <div className="hero-promises"><span><Check aria-hidden /> Un besoin clarifié avant de construire</span><span><Check aria-hidden /> Des étapes visibles jusqu’à la livraison</span></div>
          </div>
          <div className="hero-showcase">
            <div className="showcase-topline"><span>NEURALIS / DIGITAL STUDIO</span><span className="font-semibold text-cyan-800">01 — 08</span></div>
            <Image src="/images/services/sites-web.webp" alt="Deux développeurs conçoivent un site web ensemble" width={1200} height={800} sizes="(max-width: 1023px) 100vw, 50vw" preload className="hero-main-image" />
            <div className="showcase-caption"><span>Des interfaces pensées pour les personnes qui les utilisent.</span><MoveUpRight className="h-5 w-5 text-cyan-800" aria-hidden /></div>
            <div className="showcase-mini"><Image src="/images/services/data.webp" alt="Une équipe examine des graphiques pour orienter ses décisions" width={1200} height={800} sizes="96px" className="h-20 w-24 rounded-xl object-cover" /><div><span className="studio-kicker">Voir plus clair</span><p className="mt-2 text-sm font-semibold text-slate-900">Données & décisions</p></div></div>
          </div>
        </div>
    ...
    <section id="parcours" className="studio-section scroll-mt-24">
      <div className="studio-container">
        ...
        <div className="grid gap-4 md:grid-cols-3">{journeys.map((journey, index) => <Link key={journey.number} href={journey.href} data-tone={["cyan", "indigo", "mint"][index]} className="journey-card themed-card group"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-500">/ {journey.number}</span><ArrowUpRight className="h-5 w-5 text-cyan-800 transition group-hover:-translate-y-1 group-hover:translate-x-1 motion-reduce:transform-none" aria-hidden /></div><h3 className="mt-7 text-2xl font-semibold tracking-tight text-slate-900">{journey.title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{journey.text}</p><p className="mt-auto border-t border-slate-200 pt-5 text-xs font-semibold text-cyan-800">{journey.tag}</p><span className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-slate-900">Découvrir les solutions <ArrowUpRight className="h-4 w-4 transition group-hover:translate-x-1 motion-reduce:transform-none" aria-hidden /></span></Link>)}</div>
      </div>
    </section>

    <section id="expertises" className="studio-section studio-section-alt scroll-mt-24">
      <div className="studio-container">
        <div className="section-intro"><div><p className="studio-kicker">02 / Ce que nous faisons</p><h2 className="studio-heading mt-4">Huit expertises.<br />Un même cap : votre projet.</h2></div><div className="max-w-md"><p className="studio-copy">Des domaines complémentaires, illustrés par des situations de travail concrètes. Trouvez le service qui répond à votre besoin et découvrez comment nous pouvons avancer ensemble.</p><Link href="/services" className="studio-link mt-5">Explorer tous les services <ArrowUpRight className="h-4 w-4" aria-hidden /></Link></div></div>
    ...
    <section className="studio-section studio-section-alt"><div className="studio-container"><div className="section-intro"><div><p className="studio-kicker">04 / Des usages, pas du jargon</p><h2 className="studio-heading mt-4">Le digital prend tout son sens<br />dans votre quotidien.</h2></div><Link href="/solutions" className="studio-link">Les solutions par secteur <ArrowUpRight className="h-4 w-4" aria-hidden /></Link></div><div className="grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
      <Link href="/portfolio" data-tone="mint" className="concept-feature themed-card group"><Image src="/images/services/plateformes.webp" alt="Une commerçante consulte une tablette pour gérer son stock de vêtements" width={1200} height={800} sizes="(max-width: 767px) 100vw, 60vw" className="aspect-[16/9] w-full object-cover transition duration-700 group-hover:scale-[1.02] motion-reduce:transform-none" /><div className="p-6"><p className="studio-kicker">Concepts & cas d’usage</p><h3 className="mt-3 text-2xl font-semibold text-slate-900">À quoi pourrait ressembler votre solution ?</h3><p className="mt-3 text-sm leading-7 text-slate-600">Explorez des scénarios de sites, de plateformes et d’assistants. Des exemples pour vous projeter.</p><span className="studio-link mt-5">Explorer les concepts <ArrowUpRight className="h-4 w-4" aria-hidden /></span></div></Link>
      <div className="studio-panel themed-card flex flex-col justify-between p-7 md:p-9" data-tone="indigo"><div><Layers3 className="h-8 w-8 text-cyan-800" aria-hidden /><h3 className="mt-7 text-3xl font-semibold tracking-tight text-slate-900">Des offres claires.<br />Un projet à votre mesure.</h3><p className="studio-copy mt-4">Site web, présence digitale, plateforme, data ou formation : choisissez une base, puis adaptons-la à vos priorités.</p><div className="studio-tags mt-6"><span>Objectifs définis</span><span>Périmètre détaillé</span><span>Sur devis</span></div></div><div className="mt-8"><ButtonLink href="/offres" variant="ghost">Découvrir les offres</ButtonLink></div></div>
    </div></div></section>
